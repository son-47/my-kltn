...
def do_train(start_epoch, args, models:List[DATPS], train_loader, evaluator, optimizers:list, schedulers:list, checkpointers:list):
    ...
    best_single_top1 = {model.name: -float("inf") for model in models}
    best_single_epoch = {model.name: 0 for model in models}
    best_ensemble_top1 = -float("inf")
    best_ensemble_epoch = 0
    args.cur_step = 0
        ...
        if (((epoch > 10 and args.dataloader.dataset_name!='RSTPReid') or (epoch > 5 and args.dataloader.dataset_name=='RSTPReid') ) \
            ...
            if get_rank() == 0:
                ...
                for mmidx, model in enumerate(models):
                    ...
                    if best_single_top1[model.name] < top1:
                        best_single_top1[model.name] = top1
                        best_single_epoch[model.name] = epoch
                        arguments["epoch"] = epoch
                        checkpointers[mmidx].save("best", **arguments)
                        logger.info(f"\t[CKPT] Updated best-{model.name}.pth at epoch {epoch} with Top1={top1:.4f}")
                ...
                torch.cuda.empty_cache()
                if best_ensemble_top1 < top1:
                    best_ensemble_top1 = top1
                    best_ensemble_epoch = epoch
                    arguments["epoch"] = epoch
                    for checkpointer in checkpointers: checkpointer.save("best-ensemble", ema=True, **arguments)
                    logger.info(f"\t[CKPT] Updated best-ensemble checkpoints at epoch {epoch} with Top1={top1:.4f}")
                logger.info("\n\t==MODEL MEAN=> Validation Single Results - Epoch: {} - Top1={} \n ".format(epoch, top1) + str(table) + "===============")

    if get_rank() == 0:
        for model_name in best_single_top1:
            logger.info(f"best {model_name} R1: {best_single_top1[model_name]} at epoch {best_single_epoch[model_name]}")
        logger.info(f"best ensemble R1: {best_ensemble_top1} at epoch {best_ensemble_epoch}")
