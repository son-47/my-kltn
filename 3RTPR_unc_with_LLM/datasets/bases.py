...
def inject_noisy_correspondence(dataset, noisy_rate, noisy_file =None):
    logger = logging.getLogger("DANK!1910.dataset")
    nums = len(dataset)
    dataset_copy = dataset.copy()
    captions  = [i[3] for i in dataset_copy]
    aug_captions = [i[4] if len(i) > 4 else '' for i in dataset_copy]
    ...
    logger.info(
        "Caption summary: max_len=%d | min_len=%d | mean_len=%.2f",
        max(len_cap),
        min(len_cap),
        np.mean(np.array(len_cap))
    )
    # raise "hehe"
    noisy_inx = np.arange(nums)
    if noisy_rate > 0:
        logger.info("Noisy file: %s", noisy_file)
        random.seed(123)
    ...
    for i in range(nums):
        ...
        # pid, real_pid, image_id, image_path, text, aug_caption
        tmp = (pids[i], image_ids[i], images[i], captions[noisy_inx[i]], aug_captions[i])
        dataset[i] = tmp
...
class ImageTextMLMDataset(Dataset):
    def __init__(self,
        ...
        if self.use_augmented or self.aug_ratio > 0:
            aug_count = sum(1 for s in self.dataset if len(s) > 4 and s[4])
            logger = logging.getLogger("DANK!1910.dataset")
            logger.info(
                "Augmented captions available: %d/%d (use_augmented=%s, aug_ratio=%.3f)",
                aug_count,
                len(self.dataset),
                self.use_augmented,
                float(self.aug_ratio),
            )
    ...
    def selecting_samples(self, index_list):
        if index_list is None:
            ...
            for sidx in index_list:
                pid, image_id, img_path, caption = self.dataset[sidx][:4]
                aug_caption = self.dataset[sidx][4] if len(self.dataset[sidx]) > 4 else ''
                new_dataset.append((pid, image_id, img_path, caption, aug_caption))
            self.repaired_samples = new_dataset

    def __getitem__(self, index):
        if self.repaired_samples is None:
            sample = self.dataset[index]
            if len(sample) >= 5:
                pid, image_id, img_path, caption, aug_caption = sample
            else:
                pid, image_id, img_path, caption = sample
                aug_caption = ''
        else:
            sample = self.repaired_samples[index]
            if len(sample) >= 5:
                pid, image_id, img_path, caption, aug_caption = sample
            else:
                pid, image_id, img_path, caption = sample
        ...
        # Tokenize augmented caption from LLM-DA++ (caption_bt)
        aug_caption_tokens = tokenize(aug_caption, tokenizer=self.tokenizer, text_length=self.text_length, truncate=self.truncate) if aug_caption else caption_tokens.clone()
        masked_aug_caption_tokens_a = self.txt_data_aug(aug_caption_tokens.clone().cpu().numpy()) if aug_caption else masked_caption_tokens_a
        masked_aug_caption_tokens_b = self.txt_data_aug(aug_caption_tokens.clone().cpu().numpy()) if aug_caption else masked_caption_tokens_b
        ...
        ret = {
            ...
            'aug_caption_ids': aug_caption_tokens,
            'masked_aug_caption_ids_a': masked_aug_caption_tokens_a,
            'masked_aug_caption_ids_b': masked_aug_caption_tokens_b,
            'index':index,
            "none":True
        }
