...
def build_dataloader(
    ...
    dataset = __factory[args.dataloader.dataset_name](
        root=args.iocfg.datadir,
        use_augmented=use_augmented,
        annotation_dir=annotation_dir,
        annotation_source=annotation_source,
        annotation_train_source=annotation_train_source,
        annotation_eval_source=annotation_eval_source,
        annotation_train_dir=annotation_train_dir,
        annotation_eval_dir=annotation_eval_dir,
    )
    logger.info(
        "Build dataloader: dataset=%s root=%s annotation_dir=%s annotation_source=%s annotation_train_source=%s annotation_eval_source=%s annotation_train_dir=%s annotation_eval_dir=%s use_augmented=%s aug_ratio=%.3f",
        args.dataloader.dataset_name,
        args.iocfg.datadir,
        annotation_dir,
        annotation_source,
        annotation_train_source,
        annotation_eval_source,
    ...
    if args.training:
        ...
        logger.info(
            "Validation split: %s (images=%d, captions=%d)",
            args.dataloader.val_dataset,
            len(ds['image_pids']),
            len(ds['captions']),
        )
        if args.dataloader.val_dataset == 'val' and len(ds['image_pids']) == 0:
            logger.info("Validation loader is empty (val split has 0 samples)")
