...
def test(cfg, args):
    ...
    test_img_loader, test_txt_loader, num_classes = build_dataloader(
        cfg,
        use_augmented=args.use_augmented,
        aug_ratio=args.aug_ratio,
        annotation_dir=args.annotation_dir,
        annotation_source=args.annotation_source,
        annotation_train_source=args.train_annotation_source,
        annotation_eval_source=args.eval_annotation_source,
        annotation_train_dir=args.train_annotation_dir,
        annotation_eval_dir=args.eval_annotation_dir,
    )
    model_a = build_model(cfg, num_classes=num_classes, name="a")
    model_b = build_model(cfg, num_classes=num_classes, name="b")
    ...
    print("\n\n","============"*5, "\n\n", "\t\t Best pair")
    checkpointer_a.load(f=op.join(args.output_dir, 'best-ensemble-a-bestpair.pth'))
    checkpointer_b.load(f=op.join(args.output_dir, 'best-ensemble-b-bestpair.pth'))
    model_a.to(device)
    model_b.to(device)
    do_inference([model_a, model_b], test_img_loader, test_txt_loader)
    ...
    checkpointer_a.load(f=op.join(args.output_dir, 'best-a.pth'))
    checkpointer_b.load(f=op.join(args.output_dir, 'best-ensemble-a-bestpair.pth'))
    model_a.to(device)
    model_b.to(device)
    do_inference([model_a, model_b], test_img_loader, test_txt_loader)

    print("\n\n","============"*5, "\n\n", "\t\t Best pair merged b-bp")
    checkpointer_a.load(f=op.join(args.output_dir, 'best-b.pth'))
    checkpointer_b.load(f=op.join(args.output_dir, 'best-ensemble-b-bestpair.pth'))
    model_a.to(device)
    model_b.to(device)
    do_inference([model_a, model_b], test_img_loader, test_txt_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ccd-tiebreak", type=str, default='random', choices=['random', 'sim'],
                        help= "tiebreak strategy for consensus_division==1: random (coin-flip) or sim (higher similarity = clean)")
    ...
    # Uncertainty
    parser.add_argument("--uncertainty", action='store_true', help= "enable uncertainty-based sample weighting")
    parser.add_argument("--uncertainty-tau", type=float, default=0.1, help= "temperature for uncertainty weighting")
    parser.add_argument("--uncertainty-alpha", type=float, default=0.5, help= "blending weight between uncertainty and dynamic: final = (1-alpha)*dynamic + alpha*uncertainty")
    #APL
    ...
    # Augmentation from LLM-DA++
    parser.add_argument("--use_augmented", action="store_true", help="Sử dụng augmented captions từ LLM-DA++")
    parser.add_argument("--aug_ratio", type=float, default=0.2, help="Tỷ lệ sử dụng augmented captions thay vì captions gốc (0.0-1.0)")
    parser.add_argument("--annotation_dir", type=str, default=None, help="Thư mục chứa annotation files (vd: ./annotation/annotation)")
    parser.add_argument(
        "--annotation_source",
        type=str,
        default="auto",
        choices=["auto", "dir", "raw"],
        help="Chọn nguồn annotation: auto (ưu tiên annotation_dir/annotation), dir (bắt buộc thư mục), raw (file gốc)",
    )
    parser.add_argument(
        ...
        default=None,
        choices=["auto", "dir", "raw"],
        help="Nguồn annotation cho train (ghi de annotation_source neu set)",
    )
    parser.add_argument(
        "--eval_annotation_source",
        type=str,
        default=None,
        choices=["auto", "dir", "raw"],
        help="Nguồn annotation cho val/test (ghi de annotation_source neu set)",
    )
    parser.add_argument(
        "--train_annotation_dir",
        type=str,
        default=None,
        help="Thu muc annotation rieng cho train (ghi de annotation_dir neu set)",
    )
    parser.add_argument(
        "--eval_annotation_dir",
        type=str,
        default=None,
        help="Thu muc annotation rieng cho val/test (ghi de annotation_dir neu set)",
    )
    ...
    cfg.use_augmented = args.use_augmented
    cfg.aug_ratio = args.aug_ratio
    cfg.annotation_dir = args.annotation_dir
    cfg.annotation_source = args.annotation_source
    cfg.train_annotation_source = args.train_annotation_source
    cfg.eval_annotation_source = args.eval_annotation_source
    cfg.train_annotation_dir = args.train_annotation_dir
    cfg.eval_annotation_dir = args.eval_annotation_dir
    
    if args.ccd:
        print('[!!!] USING Confident Consensus Division with mode-remake=', args.ccd_r, " and use all model = ", args.ccd_a, " and tiebreak =", args.ccd_tiebreak)
        cfg.ccd.enable = True
        cfg.ccd.remakeds = args.ccd_r
        cfg.ccd.all_model = args.ccd_a
        cfg.ccd.tiebreak = args.ccd_tiebreak
    ...
    if args.uncertainty:
        cfg.losses.uncertainty = True
        cfg.losses.uncertainty_tau = args.uncertainty_tau
        cfg.losses.uncertainty_alpha = args.uncertainty_alpha
        print(f"[!!!] USING Uncertainty weighting with tau={args.uncertainty_tau}, alpha={args.uncertainty_alpha}")
    else:
        if args.uncertainty and not args.ccd:
            raise ValueError("[ERROR] --uncertainty requires --ccd to be enabled. Per-model similarities are computed during CCD pass.")
    ...
    if args.test: test(cfg, args)
        ...
        logger.info(
            "Config summary: dataset=%s datadir=%s annotation_dir=%s annotation_source=%s train_annotation_source=%s eval_annotation_source=%s train_annotation_dir=%s eval_annotation_dir=%s",
            cfg.dataloader.dataset_name,
            cfg.iocfg.datadir,
            args.annotation_dir,
            args.annotation_source,
            args.train_annotation_source,
            args.eval_annotation_source,
            args.train_annotation_dir,
            args.eval_annotation_dir,
        )
        logger.info(
            "Augmentation: use_augmented=%s aug_ratio=%.3f text_len=%d img_size=%s batch_size=%d",
            args.use_augmented,
            float(args.aug_ratio),
            cfg.dataloader.text_length,
            cfg.image_encoder.img_size,
            cfg.dataloader.batch_size,
        )
        logger.info(
            "CCD: enable=%s warmup_epochs=%s tiebreak=%s dynamic=%s",
            cfg.ccd.enable,
            cfg.ccd.warmup_epochs,
            cfg.ccd.tiebreak,
            cfg.losses.dynamic,
        )
        logger.info(
            "Uncertainty: enable=%s tau=%s alpha=%s",
            cfg.losses.uncertainty,
            cfg.losses.uncertainty_tau,
            cfg.losses.uncertainty_alpha,
        )
        save_train_configs(output_dir, cfg)

        # get image-text pair datasets dataloader
        train_loader, val_img_loader, val_txt_loader, num_classes = build_dataloader(
            cfg,
            use_augmented=args.use_augmented,
            aug_ratio=args.aug_ratio,
            annotation_dir=args.annotation_dir,
